...
export function Board({ tasks, projectSlug }) {
  ...
  const filteredTasks = taskList.filter((task) => {
    ...
    const matchesLabel = filterLabel
      ? task.labels.some((label) => label.id === parseInt(filterLabel))
      : true;
  ...
  return (
    <main className="board">
      <header className="navbar-end">
        ...
        <button className="button is-link m-1">
          <Link
            to="/projects/$projectSlug/backlog"
            params={{ projectSlug: taskList[0].project.slug }}
            className="link"
          >
            Backlog
          </Link>
      ...
      {taskList?.length > 0 ? (
        <h1 className="title has-text-left">
          Active project: {taskList[0].project.Title}
        </h1>
      ) : (
